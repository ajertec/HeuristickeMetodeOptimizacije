...
def evaluateNode1(node, vehicles, lanes):
    ...
    for vehicle in vehicles:
        totalVehiclesLength += vehicle[1]["length"]
    ...
    for laneIdx in range(lanesAmount):
        ...
        if node["lanes"][laneIdx] != []:
            ...
            if firstSerie == 0:
                firstVehicleIndex = node["lanes"][laneIdx][0]
                firstSerie = vehicles[firstVehicleIndex][1]["series"]
            else:
                firstVehicleIndex = node["lanes"][laneIdx][0]
                temporarySerie = vehicles[firstVehicleIndex][1]["series"]

                if temporarySerie != firstSerie:
                    seriesChangedCounter += 1
                    firstSerie = temporarySerie

            for vehicleIndex in node["lanes"][laneIdx]:

                usedCapacity += vehicles[vehicleIndex][1]["length"]

            usedCapacity += (len(node["lanes"][laneIdx]) - 1) * 0.5
    ...
    F3 = (totalUsedCapacity - usedCapacity) / \
        (totalLanesCapacity - totalVehiclesLength)
    ...
    for laneIdx in range(lanesAmount):
        ...
        if node["lanes"][laneIdx] != []:
            ...
            for vehicleId in node["lanes"][laneIdx]:
                ...
                if laneLength > 2:
                    if temporaryDepartureTime == -9999:
                        ...
                        rewardPenalty += rewardOrPenalty(
                            currentDepartureTime, temporaryDepartureTime)
            ...
            if laneLength == 2:
                ...
                rewardPenalty2 = rewardOrPenalty(
                    departureTimeLast, departureTimeFirst)
